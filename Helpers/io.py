...
def readWAV(f):
    ...
    fs, data = wavfile.read(
        f)  # could also use sr.readframes(n) :  sr.readframes doesn't seem to deliver left / right channels
...
def WAV2file(ifi, ofo=''):
    ...
    fs, data = wavfile.read(
        ifi)  # could also use sr.readframes(n) :  sr.readframes doesn't seem to deliver left / right channels
