...
def visualize_weights(y_true, save_path):
    ...
    fig, ax = plt.subplots()
    ax.bar(np.arange(weights.shape[0]), weights[:,0], width=width, color='IndianRed', label='Pitchwise balancing weight', align='edge')
    weight = np.log10(calculating_class_weights(y_true=y_true, type='over_all').T)
    ax.bar(np.arange(weight.shape[0]), weight[:,0], width=width, color='g', label='Global balancing weight', linestyle='--', alpha=0.5, align='edge')
